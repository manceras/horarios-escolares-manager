...
from app.api.deps import TimeSlotServiceDep
...
def list_time_slots(service: TimeSlotServiceDep) -> Sequence[TimeSlot]:
    return service.list_all()


@router.get("/{time_slot_id}", response_model=TimeSlotRead)
def get_time_slot(time_slot_id: int, service: TimeSlotServiceDep) -> TimeSlot:
    return service.get(time_slot_id)


@router.post("", response_model=TimeSlotRead, status_code=status.HTTP_201_CREATED)
def create_time_slot(payload: TimeSlotCreate, service: TimeSlotServiceDep) -> TimeSlot:
    return service.create(payload)


@router.patch("/{time_slot_id}", response_model=TimeSlotRead)
def update_time_slot(
    time_slot_id: int, payload: TimeSlotUpdate, service: TimeSlotServiceDep
) -> TimeSlot:
    return service.update(time_slot_id, payload)


@router.delete("/{time_slot_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_time_slot(time_slot_id: int, service: TimeSlotServiceDep) -> None:

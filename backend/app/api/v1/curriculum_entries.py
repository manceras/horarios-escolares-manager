...
from app.api.deps import AnyUser, CurriculumServiceDep, StaffUser
...
def list_curriculum_entries(
    service: CurriculumServiceDep, _user: AnyUser
) -> list[CurriculumEntryDetail]:
    return service.list_all_with_details()


@router.get("/workload", response_model=WorkloadReport)
def get_workload(service: CurriculumServiceDep, _user: AnyUser) -> WorkloadReport:
    return service.workload()


@router.get("/{entry_id}", response_model=CurriculumEntryDetail)
def get_curriculum_entry(
    entry_id: int, service: CurriculumServiceDep, _user: AnyUser
) -> CurriculumEntryDetail:
...
def create_curriculum_entry(
    payload: CurriculumEntryCreate, service: CurriculumServiceDep, _user: StaffUser
) -> CurriculumEntry:
...
def delete_curriculum_entry(entry_id: int, service: CurriculumServiceDep, _user: StaffUser) -> None:

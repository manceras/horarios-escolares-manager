...
def list_schedules(service: ScheduleServiceDep) -> Sequence[Schedule]:
    return service.list_all()


@router.post("", response_model=ScheduleRead, status_code=status.HTTP_201_CREATED)
def create_schedule(payload: ScheduleCreate, service: ScheduleServiceDep) -> Schedule:
    return service.create(payload)


@router.get("/{schedule_id}", response_model=ScheduleDetail)
def get_schedule(schedule_id: int, service: ScheduleServiceDep) -> ScheduleDetail:
    return service.get_detail(schedule_id)


@router.get("/{schedule_id}/conflicts", response_model=ConflictReport)
def get_schedule_conflicts(schedule_id: int, service: ScheduleServiceDep) -> ConflictReport:
    return service.find_conflicts(schedule_id)


@router.post("/{schedule_id}/generate", response_model=GenerationResult)
def generate_schedule(schedule_id: int, service: ScheduleServiceDep) -> GenerationResult:
...
def update_scheduled_session(
    schedule_id: int, session_id: int, payload: ScheduledSessionUpdate, service: ScheduleServiceDep
) -> ScheduledSessionRead:
    return service.update_session(schedule_id, session_id, payload)


@router.post("/{schedule_id}/publish", response_model=ScheduleRead)
def publish_schedule(schedule_id: int, service: ScheduleServiceDep) -> Schedule:
    return service.publish(schedule_id)


@router.delete("/{schedule_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_schedule(schedule_id: int, service: ScheduleServiceDep) -> None:
